...
def follow_up(run):
    df_scat = pd.DataFrame(list(zip(gain_list, begin_price, run, date_list)),
            columns = ['gains_under','begin_price_under', 'dates', 'time_elapsed_under'])
...
data = {'Daily_Pct_Changes_under' : [str(n_one) + ' & less', str(n_one)+ ' to ' + str(n_two), str(n_two) + ' to ' + str(n_three), str(n_six) + ' to ' + str(n_seven), str(n_seven) + ' to '  + str(n_eight), str(n_eight) + ' & above'], 
...
frame_1 = frame.reindex(index = frame.index[::-1])
    
df_scat_under = pd.concat([df_scat, frame_1], axis = 1)

spread = Spread('Stock_Dashboard')

# spread.df_to_sheet(df_scat_under, index = False, sheet = 'Under_200_1', replace = True)

# spread.df_to_sheet(frame_1, index = False, start = 'A11', sheet = 'Chance_Dash', replace = False)
...
def follow_up(run):
    df_scat = pd.DataFrame(list(zip(gain_list, begin_price, run, date_list)),
            columns = ['gains_over','begin_price_over', 'dates', 'time_elapsed_over'])
...
# # ------------
data = {'Daily_Pct_Changes_over' : [str(n_one) + ' & less', str(n_one)+ ' to ' + str(n_two), str(n_two) + ' to ' + str(n_three), str(n_six) + ' to ' + str(n_seven), str(n_seven) + ' to '  + str(n_eight), str(n_eight) + ' & above'], 
...
frame_2 = frame.reindex(index = frame.index[::-1])

df_scat_over = pd.concat([df_scat, frame_2], axis = 1)

# spread.df_to_sheet(df_scat_over, index = False, sheet = 'Over_200_1', replace = True)

# spread.df_to_sheet(frame_2, index = False, start = 'A2', sheet = 'Chance_Dash', replace = False)
...
given_freq.sort_values(by = 'WEIGHTING', ascending = False, inplace = True)
...
frame_3 = frame.reindex(index = frame.index[::-1])
        
df_scat_under = pd.concat([df_scat, frame_3], axis = 1)

spread = Spread('Stock_Dashboard')

# spread.df_to_sheet(df_scat_under, index = False, sheet = 'Under_200_2', replace = True)

# spread.df_to_sheet(frame_3, index = False, start = 'A29', sheet = 'Chance_Dash', replace = False)
...
frame_4 = frame.reindex(index = frame.index[::-1])

df_scat_over = pd.concat([df_scat, frame_4], axis = 1)

# spread.df_to_sheet(df_scat_over, index = False, sheet = 'Over_200_2', replace = True)

# spread.df_to_sheet(frame_4, index = False, start = 'A20', sheet = 'Chance_Dash', replace = False)

symbol = [sym, sym, sym, sym, sym, sym, sym, sym, sym, sym]
symbol = pd.Series(symbol)

symbol_2 = [sym, sym, sym, sym, sym, sym, sym, sym, sym, sym, sym, sym]
symbol_2 = pd.Series(symbol_2)

given_freq.insert(loc = 0, column = 'Symbol', value = symbol_2)

over = pd.concat([frame_4, frame_2])
over.insert(loc = 0 , column = 'Symbol', value = symbol)
over.reset_index(drop = True, inplace = True)

under = pd.concat([frame_1, frame_3])
under.insert(loc = 0 , column = 'Symbol', value = symbol)
under.reset_index(drop = True, inplace = True)

engine = sqlalchemy.create_engine('mysql+pymysql://root:badass11@localhost:3306/Stock_Insight')
